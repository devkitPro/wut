...
extern "C" IUpdatePackageAccessorInternal *
...
public:
   ...
   virtual bool
   PackageExists() const = 0;

   virtual bool
   IsUpdatePackageDownloaded() const = 0;

private:
   virtual details::IUpdatePackageAccessorInternal *
   GetInternal() = 0;
};
...
{
public:
   explicit UpdatePackageAccessorFromPtr(details::IUpdatePackageAccessorInternal *ptr) :
      mInstancePtr(ptr)
   {
   }
   bool
   PackageExists() const override
   {
      if (!mInstancePtr) {
         return false;
      }
   ...
   bool
   IsUpdatePackageDownloaded() const override
   {
      if (!mInstancePtr) {
         return false;
      }
