...
public:
   ...
   virtual nn::Result
   Get(TitleInfo *outTitleInfo, uint32_t *outTitleInfoNum, int maxNumTitleInfos, uint32_t u1) const = 0;

private:
   virtual details::IPreferentialTitleAccessorInternal *
   GetInternal() = 0;
};
...
{
public:
   explicit PreferentialTitleAccessorFromPtr(details::IPreferentialTitleAccessorInternal *ptr) :
      mInstancePtr(ptr)
   {
   }
   nn::Result
   Get(TitleInfo *outTitleInfo, uint32_t *outTitleInfoNum, int maxNumTitleInfos, uint32_t u1) const override
   {
      if (!mInstancePtr) {
