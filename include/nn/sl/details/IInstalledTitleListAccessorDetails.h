...
public:
   ...
   virtual nn::Result
   Get(nn::sl::TitleInfo *outTitleInfos, int *outTitleInfosSize, int maxTitleInfos) const = 0;

private:
   virtual details::IInstalledTitleListAccessorInternal *
   GetInternal() = 0;
};
...
{
public:
   explicit InstalledTitleListAccessorFromPtr(details::IInstalledTitleListAccessorInternal *ptr) :
      mInstancePtr(ptr)
   {
   }
   nn::Result
   Get(nn::sl::TitleInfo *outTitleInfos, int *outTitleInfosSize, int maxTitleInfos) const override
   {
      if (!mInstancePtr) {
