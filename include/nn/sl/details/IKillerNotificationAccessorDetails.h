...
public:
   ...
   virtual nn::Result
   Get(KillerNotification *outKillerNotifications, int *outKillerNotificationsNum, int maxKillerNotifications) const = 0;

private:
   virtual details::IKillerNotificationAccessorInternal *
   GetInternal() = 0;
};
...
{
public:
   explicit KillerNotificationAccessorFromPtr(details::IKillerNotificationAccessorInternal *ptr) :
      mInstancePtr(ptr)
   {
   }
   nn::Result
   Get(nn::sl::KillerNotification *outKillerNotifications, int *outKillerNotificationsNum, int maxKillerNotifications) const override
   {
      if (!mInstancePtr) {
