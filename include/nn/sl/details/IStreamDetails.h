...
public:
   ...
   virtual nn::Result
   Read(uint32_t *bytesRead, void *buffer, uint32_t readSize) = 0;
   virtual nn::Result
   Write(uint32_t *bytesWritten, void *buffer, uint32_t writeSize) = 0;
   virtual nn::Result
   GetSize(uint32_t *fileSize) = 0;
   virtual nn::Result
   Seek(int32_t offset, nn::sl::SeekOrigin seekOrigin) = 0;

private:
   virtual details::IStreamInternal *
   GetInternal() = 0;
};
