...
public:
   ...
   virtual nn::Result
   Update(nn::sl::TitleInfo *titleInfos, int num) = 0;

   virtual nn::Result
   Store() = 0;

   virtual void
   Get(nn::sl::IconInfo *iconInfos, int num) const = 0;

private:
   virtual details::ITitleIconCacheInternal *
   GetInternal() = 0;
};
...
{
public:
   explicit TitleIconCacheFromPtr(details::ITitleIconCacheInternal *ptr) :
      mInstancePtr(ptr)
   {
   }
   nn::Result
