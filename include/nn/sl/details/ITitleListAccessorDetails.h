...
public:
   ...
   virtual nn::Result
   Get(nn::sl::TitleInfo *outTitleInfos, int *outTitleInfosNum, int maxTitleInfosNum) const = 0;

private:
   virtual details::ITitleListAccessorInternal *
   GetInternal() = 0;
};
...
{
public:
   explicit TitleListAccessorFromPtr(details::ITitleListAccessorInternal *ptr) :
      mInstancePtr(ptr)
   {
   }
   nn::Result
   Get(nn::sl::TitleInfo *outTitleInfos, int *outTitleInfosNum, int maxTitleInfosNum) const override
   {
      if (!mInstancePtr) {
