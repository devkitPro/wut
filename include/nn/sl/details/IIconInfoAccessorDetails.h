...
public:
   ...
   virtual nn::Result
   GetTitleIconInfo(nn::sl::IconInfo *outIconInfo, const nn::sl::TitleInfo &titleInfo, nn::sl::Language language) const = 0;

   virtual nn::Result
   GetMiiIcon(void *buffer, uint32_t buffer_size, uint32_t slot) const = 0;

private:
   virtual details::IIconInfoAccessorInternal *
   GetInternal() = 0;
};
...
{
public:
   explicit IconInfoAccessorFromPtr(details::IIconInfoAccessorInternal *ptr) :
      mInstancePtr(ptr)
   {
   }

   nn::Result
   GetTitleIconInfo(nn::sl::IconInfo *outIconInfo, const nn::sl::TitleInfo &titleInfo, nn::sl::Language language) const override
   {
      if (!mInstancePtr) {
   ...
   nn::Result
   GetMiiIcon(void *buffer, uint32_t buffer_size, uint32_t slot) const override
   {
      if (!mInstancePtr) {
         return {Result::LEVEL_FATAL, Result::RESULT_MODULE_NN_SL, 1};
      }
