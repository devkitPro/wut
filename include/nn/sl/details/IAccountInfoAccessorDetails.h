...
public:
   ...
   virtual nn::Result
   Get(nn::sl::AccountInfo *) const = 0;

private:
   virtual details::IAccountInfoAccessorInternal *
   GetInternal() = 0;
};
...
{
public:
   explicit AccountInfoAccessorFromPtr(details::IAccountInfoAccessorInternal *ptr) :
      mInstancePtr(ptr)
   {
   }
   nn::Result
   Get(nn::sl::AccountInfo *outAccountInfo) const override
   {
      if (!mInstancePtr) {
