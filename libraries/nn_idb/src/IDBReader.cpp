...
nn::Result
IDBReader::LoadIconDataFromIdb(uint64_t titleId, IconDataRaw &iconData) const
{
...
bool
IDBReader::SearchFromDataBase(IconDataKey &key, uint32_t &outIndex) const
{
   if (!mInitDone) {
      return false;
   }
...
bool
IDBReader::LoadIconDataFromDatabase(uint32_t index, IconDataRaw &iconData) const
{
   if (!mInitDone) {
      return false;
   }
...
bool
IDBReader::LoadIconData(uint64_t titleId, uint32_t u1, IconDataRaw &iconData) const
{
   if (!mInitDone) {
      return false;
   }
...
bool
IDBReader::Initialize(const std::string &path)
{
   this->mPath = path;
   if (!mFileHeader) {
...
int
IDBReader::LoadFromFile(void *buffer, uint32_t size, uint32_t offset) const
{
   if (!mFile) {
      return 0;
   }
